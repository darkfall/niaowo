...
#define JOIN(a, b) _JOIN_X(a, b)
#define _JOIN_X(a, b) a##b

#define HEComputeDigest(method)						\
	JOIN(JOIN(CC_, method), _CTX) _ctx;								\
	unsigned char digest[JOIN(JOIN(CC_, method), _DIGEST_LENGTH)];		\
	CC_##method##_Init(&_ctx);							\
	CC_##method##_Update(&_ctx, [self bytes], (unsigned int)[self length]);		\
	CC_##method##_Final(digest, &_ctx);

#define HEComputeDigestNSData(method)				\
	HEComputeDigest(method)						\
	return [NSData dataWithBytes:digest length:JOIN(JOIN(CC_, method), _DIGEST_LENGTH)];
